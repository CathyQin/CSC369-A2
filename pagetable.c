...
int allocate_frame(pgtbl_entry_t *p) {
	...
	if(frame == -1) { // Didn't find a free page.
		...
		struct frame coremap_frame = coremap[frame];
		if (coremap_frame.pte->frame & PG_DIRTY) {
			int offset = swap_pageout(frame, coremap[frame].pte->swap_off);

			if (offset != INVALID_SWAP) {
				coremap_frame.pte->swap_off = offset;
			} else {
		...
		coremap_frame.pte->frame &= ~PG_DIRTY;
		coremap_frame.pte->frame |= PG_ONSWAP;
		coremap_frame.pte->frame &= ~PG_VALID;
	}
